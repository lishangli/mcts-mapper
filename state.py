...
class Environment:
    ...
    def route_update(self, action):
        ...
        if inN not in self.route_cur_node:
            ...
            if outN != self.last_action % self.adg_actions:
                self.route_cur_node.append(outN)
                edges = self.adg.getEdges()
                dst_ids, src_ids, edge_ids = np.array(
                    [
                        (edge.getDstId(), edge.getSrcId(), id)
                        for id, edge in edges.items()
                    ]
                ).T

                # 使用布尔索引更新 route_mask，只更新大于 0 的值
                out_edges = edge_ids[
                    (src_ids == outN) & (self.route_mask[edge_ids] >= 0)
                ]
                in_edges = edge_ids[(src_ids == inN) & (self.route_mask[edge_ids] >= 0)]

                reversed_edges = edge_ids[
                    (dst_ids == inN) & (self.route_mask[edge_ids] >= 0)
                ]
                # print("info:  add outN {}".format(outN))
                # print("info:  remove inN {}".format(inN))
                self.route_mask[out_edges] = 1
                self.route_mask[in_edges] = 0
                self.route_mask[reversed_edges] = 0

                if self.adg.getNode(outN).getType() != "GIB":
                    print("{} not GIB\n".format(outN))
                    return False
            else:
                # print("adg success {}".format(outN))
    ...
    def updateMeta(self, action):
        ...
        predecessor_nodes = self.find_predecessor_nodes(dfg_node_id)
...
class MappingState:
    ...
    def get_distance(self, cur_action):
        ...
        predecessors = self.env.find_predecessor_nodes(current_dfg_id)
        distance = 0
    ...
    def take_action(self, action, print_action=False):
        ...
        if not self.env.update(action, self.reward, print_action):
            ...
            self.sum_reward -= 500
        elif self.is_terminal() and self.env.dfg_mask.any():
            self.mapping_state = False
            self.sum_reward -= 500
        else:
            self.last_reward = self.sum_reward
            self.sum_reward -= self.get_distance(action) + 1
